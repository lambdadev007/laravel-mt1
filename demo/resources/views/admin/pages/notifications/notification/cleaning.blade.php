...
<div class="notification-info">
    ...
    <div class="info-row">
        ...
        <span>{{ json_decode($data['notification']['information'])->price }} <span class="dire-lari"></span></span>
    </div>
    <div class="info-row border-bottom-0">
        <span class="title">სერვისი</span>
    </div>
    <div id="services-accordion">
        @php
            if ( json_decode($data['notification']['information'])->table == 'top' ) {
                $obj = CleaningTopServices::where('id', json_decode($data['notification']['information'])->id)->get()->toArray();
                $obj = $obj[0];
            } else {
                $obj = CleaningBottomServices::where('id', json_decode($data['notification']['information'])->id)->get()->toArray();
                $obj = $obj[0];
            }
        @endphp
        <button class="notification-collapse-button" type="button" data-toggle="collapse" data-target="#service" aria-expanded="false" aria-controls="service">
            სერვისის იდენტიფიკატორი: {{ $obj['id'] }} <span class="dire-right-arrow"></span>
        </button>

        <div class="notification-collapse details collapse flex-column" id="service" aria-labelledby="service" data-parent="#services-accordion">
            <div class="info-row">
            ...
            <div class="info-row">
                ...
                <span class="cut-text">{!! $obj['description_'. Session::get('locale')] !!}</span>
            </div>
            <div class="info-row">
                <span>სერვისის ფასი:</span>
                <span>1 კვ.მ {{ $obj['price'] }} <span class="dire-lari"></span></span>
            </div>

            <button class="notification-collapse-button" type="button" data-toggle="collapse" data-target="#images" aria-expanded="false" aria-controls="images">
                სურათები <span class="dire-right-arrow"></span>
            </button>

            <div class="notification-collapse details collapse flex-column" id="images" aria-labelledby="images">
                @if ( json_decode($data['notification']['information'])->images != [] )
                    @foreach ( json_decode($data['notification']['information'])->images as $image)
                        <div class="info-row cleaning-images">
                            <img src="{{ asset($image) }}">
                        </div>
                    @endforeach
                @endif
