            ...
            <div class="left d-fc">
                @foreach ( $product_categories['main'] as $main_index => $main )
                    <div class="{{ $main['has'] }} {{ ($main_index == 0) ? 'active' : '' }}">
                        <button type="button" class="toggle-all-categories-popup" data-target="{{ $main['has'] }}">{{ $main['title'] }}</button>
                    </div>
                @endforeach
            </div>
            <div class="right">
                @foreach ( $product_categories['main'] as $main_index => $main )
                    <div class="groups-wrapper {{ $main['has'] }} {{ ($main_index == 0) ? '' : 'hidden' }}">
                        @foreach ( $product_categories['groups'] as $group_index => $group )
                            @if ( $group['belongs'] == $main['has'] )
                                <div class="category">
                                    <div class="title">
                                        <a href="/enter/product/select?group={{ $group['has'] }}">{{ $group['title'] }}</a>
                                    ...
                                    <div class="links d-fc">
                                        @foreach ( $product_categories['sub_groups'] as $sub_group_index => $sub_groups )
                                            @if ( $sub_groups['belongs'] == $group['has'] )
                                                <a href="/enter/product/select?group={{ $sub_groups['belongs'] }}&category={{ $sub_groups['search_id'] }}">{{ $sub_groups['title'] }}</a>
                                            @endif
                                        @endforeach
                                    </div>
                                </div>
