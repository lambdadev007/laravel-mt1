...
@endphp
@php
$text=pdf::where('status',1)->get()->toArray();

@endphp
@foreach($text as $txt) 

{!! html_entity_decode($txt['pdf_content']) !!}
@endforeach  
<!-- <style type="text/css">
...
<p class="my-5">დირექტორი: კრისტი დევაძე</p>
    ...
    <tr>
        ...
        <td class="border py-2 px-3 w-50">
            @if ( $data['type'] == 'individual' )
                სახელი და გვარი: {{ $data['name'] }}<br>
                პირადი ნომერი: {{ $data['p_id'] }}<br>
            @else
                დასახელება: {{ $data['name'] }}<br>
                საიდენტიფიკაციო ნომერი: {{ $data['p_id'] }}<br>
            @endif
            ტელეფონი: {{ $data['phone_number'] }}<br>
        </td>
    </tr>
</table> -->
...
<!-- <table class="w-100 mb-4">
    <tr>
       <td class="border border-right-0 py-2 px-3 w-50">
        ...
        <td colspan="2" class="border py-2 px-3 w-50">
        //{ html_entity_decode($txt['pdf_content']) }
        </td>
    </tr>
</table> -->
